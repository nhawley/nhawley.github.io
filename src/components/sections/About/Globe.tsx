...
const DEFAULT_MARKERS = [
  ...
  { lat: 40.71, lng: -74.01, label: "New York City" },
...
function project(
  ...
  return [cx - x * scale, y * scale + cy, z];
}
...
export function Globe({
  ...
  const rotYRef = useRef(3);
  const rotXRef = useRef(1);
  ...
  const arcColorRef = useRef("");
  const markerColorRef = useRef("");

  useEffect(() => {
    const resolve = () => {
      const isDark = document.documentElement.classList.contains("dark");
      dotColorRef.current = isDark
        ? "rgba(100, 180, 255, ALPHA)"
        : "rgba(30, 80, 160, ALPHA)";
      arcColorRef.current = isDark
        ? "rgba(100, 180, 255, 0.5)"
        : "rgba(30, 80, 160, 0.5)";
      markerColorRef.current = isDark
        ? "rgba(100, 220, 255, 1)"
        : "rgba(30, 80, 180, 1)";
    };
    resolve();
  ...
  const draw = useCallback(() => {
    ...
    for (let i = 0; i < dots.length; i++) {
      ...
      ctx.fillStyle = dotColorRef.current.replace("ALPHA", depthAlpha.toFixed(2));
      ctx.fill();
    }
    ...
    for (const conn of connections) {
      ...
      ctx.strokeStyle = arcColorRef.current;
      ctx.lineWidth = 1.2;
      ctx.stroke();
    ...
    for (const marker of markers) {
      ...
      const mc = markerColorRef.current;
      ctx.strokeStyle = mc.replace("1)", `${0.2 + pulse * 0.15})`);
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      if (marker.label) {
        ...
        ctx.fillStyle = mc.replace("1)", "0.6)");
